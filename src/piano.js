...
class Piano {
   ...
   *  sustain    sustain
   *
   * evt: event generated by WebMIDI
   *
  ...
  noteOn (number, velocity) {
    ...
    Object.keys(store.state.players).forEach(name => {
      ...
      if (player.noteOn) {
        player.noteOn.call(player.scope, number, velocity)
      }
    })
  }
  ...
  noteOff (number) {
    ...
    Object.keys(store.state.players).forEach(name => {
      var player = store.state.players[name]
      if (player.noteOff) {
        player.noteOff.call(player.scope, number, this.keys[number])
      }
    })
  }
   ...
   *
   * pedal := evt.controller.name
   *          allowed values ['holdpedal', 'sustenutopedal', 'softpedal']
   * value := evt.value
   *          allowed values [0, 128]
   */
  pedal (pedal, value) {
